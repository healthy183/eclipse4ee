...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public void deleteById(Serializable id) {
		String sql = " DELETE FROM " + entityClass.getSimpleName() + " WHERE "+pkName+" = ?";
	...
	public T findById(Serializable id) {
		String sql = "SELECT * FROM " + entityClass.getSimpleName() + " WHERE "+pkName+" = ?";
	...
	private String makeSql(String sqlFlag) {
		...
		if (sqlFlag.equals(SQL_INSERT)) {
			...
			for (int i = 0; fields != null && i < fields.length; i++) {
				fields[i].setAccessible(true); // 暴力反射
				String column = fields[i].getName();
				if (column.equals(pkName)) { // id 代表主键
					continue;
				}
			...
			sql.append(" DELETE FROM " + entityClass.getSimpleName() + " WHERE "+pkName+" =?");
		}
	...
	private List<T> find(int pageNo, int pageSize, Map<String, String> where, LinkedHashMap<String, String> orderby) {
		// where 与 order 要写在select * from table 的后面，而不是where rownum<=? ) where rn>=?的后面
